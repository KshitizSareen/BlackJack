...
void BlackJackCards::UI()
{
	...
	InitialBet=InitialBet-Amount; //Deduct the total amount from the amount you bet
	return;
}
void BlackJackCards::dealerCards()
{

	UpcardNumber=Numbers[rand()%Numbers.size()]; //Select Upcard Number of dealer
	for(itr=CardValues.begin();itr!=CardValues.end();itr++)
	{
		if(itr->first==UpcardNumber)
		UpcardValue=itr->second; //Mapping string to integer
	}
	UpcardType=Types[rand()%Types.size()];//Select Upcard Type of dealer
	hiddencardNumber=Numbers[rand()%Numbers.size()];//Select HiddenCard Number of dealer
	for(itr=CardValues.begin();itr!=CardValues.end();itr++)
	{
		if(itr->first==hiddencardNumber)
		hiddencardValue=itr->second;//Mapping string to integer
	}
	hiddencardType=Types[rand()%Types.size()]; //Select HiddenCard Type of dealer
...
int BlackJackCards::playerCards()
{
	...
	FirstcardNumber=Numbers[rand()%Numbers.size()]; //Select FirstCard Number of Player
	for(itr=CardValues.begin();itr!=CardValues.end();itr++)
	{
		if(itr->first==FirstcardNumber)
		FirstcardValue=itr->second;//Mapping string to integer
	}
	FirstcardType=Types[rand()%Types.size()];//Select FirstCard Type of Player
	SecondcardNumber=Numbers[rand()%Numbers.size()];//Select SecondCard Number of Player
	for(itr=CardValues.begin();itr!=CardValues.end();itr++)
	{
		if(itr->first==SecondcardNumber)
		SecondcardValue=itr->second;//Mapping string to integer
	}
	SecondcardType=Types[rand()%Types.size()];//Select SecondCard Type of Player
	...
	sumPlayer=FirstcardValue+SecondcardValue; //Getting the value of the player
	cout<<"The sum of the player cards are "<<sumPlayer<<"\n";
	if(sumPlayer==21)
	{
	cout<<"BlackJack"<<"\n"<<"The player wins"<<"\n";
	Amount=Amount*2;
	InitialBet=InitialBet+Amount; //Incrementing the initial bet with the amount won
...
void BlackJackCards::Hit()
{
    ...
    NewCardNumber=Numbers[rand()%Numbers.size()]; //Select NewCard Number of Player
    NewCardType=Types[rand()%Types.size()];//Select NewCard Type of Player
    cout<<"The new card is "<<NewCardNumber<<"-"<<NewCardType<<"\n";
    for(itr=CardValues.begin();itr!=CardValues.end();itr++)
	{
		if(itr->first==NewCardNumber)
        {
            NewCardValue=itr->second;//Mapping string to integer
        }
	}
	NewCards.insert(NewCards.begin(),NewCardValue);
	sumPlayer=sumPlayer+NewCardValue; //Getting the new total value after a hit
...
void BlackJackCards::Stand()
{
    ...
    sumDealer=UpcardValue+hiddencardValue; //Getting the total value of the dealer
    cout<<"The dealer value is "<<sumDealer<<"\n"<<"The player value is "<<sumPlayer<<"\n";
    if(sumDealer<sumPlayer)
    {
        while(sumDealer<sumPlayer)
        {
        NewCardNumber=Numbers[rand()%Numbers.size()]; //Select NewCard Number of dealer
    NewCardType=Types[rand()%Types.size()];//Select NewCard Type of dealer
	...
	NewCards.insert(NewCards.begin(),NewCardValue);
	sumDealer=sumDealer+NewCardValue; //Getting the total value of the dealer after obtaining a new card
...
void BlackJackCards::DoubleDown()
{
    ...
    Amount=Amount*2; //Doubling the amount that was bet initially
    InitialBet=InitialBet-(Amount/2); //Decrementing the total amount by the total amount bet after doubling down
...
{
	NewCardNumber=Numbers[rand()%Numbers.size()];//Getting the new card number of the first split
    NewCardType=Types[rand()%Types.size()];//Getting the new card Type of the first split
    for(itr=CardValues.begin();itr!=CardValues.end();itr++)
	{
		if(itr->first==NewCardNumber)
        {
	...
	SplitSecondCardValue=SecondcardValue;//Saving the second car for the second split
	SplitSecondCardNumber=SecondcardNumber;
	SecondcardValue=NewCardValue;
	SecondcardNumber=NewCardNumber;
	sumPlayer=FirstcardValue+SecondcardValue;//Getting the total sum of the player in the first split
	DifferentCases();//Check the cases of the first split
    Options();//Input the valid option
    SelectOptions();//Select the option that you input
	NewCardNumber=Numbers[rand()%Numbers.size()];//Getting the new card number of the Second split
    NewCardType=Types[rand()%Types.size()];//Getting the new card type of the Second split
    for(itr=CardValues.begin();itr!=CardValues.end();itr++)
	{
		if(itr->first==NewCardNumber)
        {
	...
	FirstcardValue=SplitSecondCardValue;//Using the second as the first card of the second split
	FirstcardNumber=SplitSecondCardNumber;
	SecondcardValue=NewCardValue;
	SecondcardNumber=NewCardNumber;
	sumPlayer=FirstcardValue+SecondcardValue;//Getting the total value of the second split
	DifferentCases();//Check the cases of the Second split
    Options();//Input the valid option
    SelectOptions();//Select the option that you input
    return;
}
